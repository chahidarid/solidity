...
pair<shared_ptr<Block>, shared_ptr<AsmAnalysisInfo>> parse(string const& _source)
{
	...
	shared_ptr<Scanner> scanner = make_shared<Scanner>(CharStream(_source, "--INPUT--"));
	shared_ptr<Block> parserResult = Parser(errorReporter, EVMDialect::strictAssemblyForEVM()).parse(scanner, false);
	if (parserResult)
	{
		shared_ptr<AsmAnalysisInfo> analysisInfo = make_shared<AsmAnalysisInfo>();
		yulAssert(errorReporter.errors().empty(), "Parsed successfully but had errors.");
		if (AsmAnalyzer(
			*analysisInfo,
			errorReporter,
		...
		{
			yulAssert(errorReporter.errors().empty(), "Analyzed successfully but had errors.");
			return make_pair(std::move(parserResult), std::move(analysisInfo));
		}
	}
	printErrors(errors);
	return {};
